package com.zircky.rotarycraft_unofficial.api.library;
...
public class EngLibrary {
  ...
  public static boolean mat_twistfailure(int torque, double radius, double taumax) {
    double J = (pi/2)* MathLibrary.intpow(radius,4);
  ...
  public static boolean mat_rot_tensfailure(double m, int rpm, double radius, double A, double sigmamax) {
    ...
    double Ftens = omega*m*radius/ MathLibrary.intpow(radius, 2);
  ...
  public static double rotenergy(double rho, double t, double omega, double radius) {
    ...
    double mass = rho*V;
    double I = mass*(pi/4)* MathLibrary.intpow(radius, 4);
    return 0.5*I* MathLibrary.intpow(omega, 2);
  }
  ...
  public static String getSIPrefix(double val) {
    if (val == 0 || (val < 10 && val >= 1))
      return "";
    // Заменяем MathHelper.floor_double на Math.floor (устаревший метод удален в 1.20.1)
    int log = (int) Math.floor(MathLibrary.logbase(val, 1000));
    switch(log) {
      case 1:
        return "k";
      case 2:
